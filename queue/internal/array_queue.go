...
//queue with array implementation: Circular Queue
type Queue struct {
	Front int
	Rear int
	Capacity int
...
func (queue *Queue) ReSize(additionalCapacity int) {
	...
	//handle rear is behind the front
	if queue.Front>queue.Rear{
		//copy existing queue with same order of sequence
		for index,elem:=range queue.Elements{
			nElemens[index+size]=elem
		}
		queue.Rear=queue.Rear+additionalCapacity
	}else{
		//copy existing queue with same order of sequence
		for index,elem:=range queue.Elements{
			nElemens[index]=elem
